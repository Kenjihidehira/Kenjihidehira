...
write("proof-panel-fit.svg", shell(900, 300, C.coral,
  `${safeBox(26, 58, 294, 214, `${text(22, 30, "PORTF&#211;LIO EM N&#218;MEROS", 12, C.coral, 850, MONO)}${text(22, 78, "Evid&#234;ncia antes", 26, C.text, 950)}${text(22, 112, "de promessa.", 26, C.text, 950)}${text(22, 154, "Projetos publicados, interfaces", 13, C.muted, 550)}${text(22, 176, "distintas e valida&#231;&#227;o objetiva.", 13, C.muted, 550)}${text(22, 201, "AUDITORIA / 2026.07", 10, C.dim, 700, MONO)}`, { fill: C.surface, stroke: C.border, radius: 6, accent: C.coral })}
...
function projectCard(name, project) {
  const right = `${text(18, 28, project.panel, 11, C.muted, 750, MONO)}${text(18, 58, project.metric, 22, project.accent, 900)}${chart(project.chart, project.accent)}${text(18, 176, project.status, 10, project.accent, 750, MONO)}`;
  const left = `${text(22, 30, `${project.code} / ${project.category}`, 12, project.accent, 850, MONO)}${text(22, 76, project.title, 31, C.text, 950)}${text(22, 112, project.description[0], 15, C.muted, 550)}${text(22, 136, project.description[1], 15, C.muted, 550)}${line(22, 148, 408, 148, C.border)}${text(22, 172, project.stack, 11, C.text, 800, MONO)}${text(344, 172, "ABRIR &#8599;", 11, project.accent, 850, MONO)}`;
  write(name, shell(720, 280, project.accent,
    `${safeBox(22, 54, 430, 190, left, { fill: C.surface, stroke: C.border, radius: 6 })}${safeBox(474, 54, 222, 190, right, { fill: C.surface, stroke: C.border, radius: 6 })}${text(24, 25, "SISTEMA COMERCIAL", 10, C.dim, 700, MONO)}`));
}

const cases = [
  ["case-returnops-v3.svg", { code: "CASE 01", category: "P&#211;S-VENDA", title: "ReturnOps", description: ["RMA, reembolso, estoque e risco", "de SLA em uma central operacional."], stack: "NODE.JS / API REST / PLAYBOOKS / TESTES", accent: C.coral, panel: "DEVOLU&#199;&#213;ES EM RISCO", metric: "12 priorit&#225;rias", chart: "queue", status: "SLA MONITORADO" }],
  ["case-servicehub-v3.svg", { code: "CASE 02", category: "SERVI&#199;OS", title: "ServiceHub", description: ["Agenda, clientes, funil e cobran&#231;as", "conectados em um fluxo comercial."], stack: "NODE.JS / CRM / AGENDA / AUTOMA&#199;&#195;O", accent: C.blue, panel: "OCUPA&#199;&#195;O DA AGENDA", metric: "84%", chart: "ring", status: "LEMBRETES ATIVOS" }],
  ["case-leadops-v3.svg", { code: "CASE 03", category: "MARKETING", title: "LeadOps", description: ["ROI, atribui&#231;&#227;o, pontua&#231;&#227;o", "e automa&#231;&#245;es para vendas."], stack: "NODE.JS / ANALYTICS / FUNIL / API", accent: C.yellow, panel: "ROI / 30 DIAS", metric: "+32,4%", chart: "line", status: "ATRIBUI&#199;&#195;O ATIVA" }],
  ["case-fieldops-v3.svg", { code: "CASE 04", category: "OPERA&#199;&#213;ES", title: "FieldOps", description: ["Equipe, materiais, faturamento", "e margem de projetos de campo."], stack: "PHP / TYPESCRIPT / MARGEM / RELAT&#211;RIOS", accent: C.green, panel: "MARGEM POR PROJETO", metric: "28,6%", chart: "bars", status: "CUSTO CONTROLADO" }],
];
for (const [name, project] of cases) projectCard(name, project);

const commercial = [
  ["card-vendoraudit-v2.svg", { code: "01", category: "CONFORMIDADE", title: "VendorAudit", description: ["Risco, documentos e renova&#231;&#245;es", "de fornecedores sob controle."], stack: "NODE.JS / SLA / RISCO", accent: C.coral, panel: "MATRIZ DE RISCO", metric: "08 alertas", chart: "queue", status: "RENOVA&#199;&#195;O PROTEGIDA" }],
  ["card-cobreflow-v2.svg", { code: "02", category: "FINANCEIRO", title: "CobreFlow", description: ["Receb&#237;veis, concilia&#231;&#227;o e", "cobran&#231;a ativa priorizada."], stack: "NODE.JS / FINAN&#199;AS / FILAS", accent: C.yellow, panel: "CARTEIRA", metric: "R$ 126 mil", chart: "bars", status: "RISCO PRIORIZADO" }],
  ["card-logixops-v2.svg", { code: "03", category: "LOG&#205;STICA", title: "LogixOps", description: ["SLA, incidentes e &#250;ltima milha", "em uma torre de controle."], stack: "NODE.JS / ROTAS / SLA", accent: C.green, panel: "ROTAS ATIVAS", metric: "42 entregas", chart: "line", status: "OPERA&#199;&#195;O AO VIVO" }],
  ["card-erp-v2.svg", { code: "04", category: "ESTOQUE", title: "StockPilot ERP", description: ["Ruptura, reposi&#231;&#227;o e compras", "guiadas por n&#237;vel de estoque."], stack: "NODE.JS / API / TESTES", accent: C.blue, panel: "N&#205;VEL DE ESTOQUE", metric: "17 SKUs", chart: "bars", status: "LOTE DE COMPRA PRONTO" }],
  ["card-helpdesk-v2.svg", { code: "05", category: "ATENDIMENTO", title: "ResolveDesk", description: ["Chamados, prioridade e risco", "de SLA em uma fila operacional."], stack: "NODE.JS / SLA / AUTOMA&#199;&#195;O", accent: C.coral, panel: "FILA DE CHAMADOS", metric: "06 urgentes", chart: "queue", status: "SLA MONITORADO" }],
  ["card-linkpulse-v2.svg", { code: "06", category: "MARKETING", title: "LinkPulse", description: ["Links, cliques, QR e convers&#227;o", "organizados por campanha."], stack: "NODE.JS / ANALYTICS / API", accent: C.blue, panel: "CLIQUES / 7 DIAS", metric: "+18,2%", chart: "line", status: "CONVERS&#195;O MEDIDA" }],
];
